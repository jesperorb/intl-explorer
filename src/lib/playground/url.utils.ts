...
export const parseSchemaFromURL = <Method extends FormatMethodsKeys>(): PlaygroundSchema<Method> | undefined => {
	const params = browser ? new URLSearchParams(window.location.search).get("schema") : undefined;
	const parsedParams = params ? JSON.parse(atob(params)).schema as PlaygroundSchema<Method> : undefined;
